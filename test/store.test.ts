...
function open(dir: string): ChannelStore {
...
test("post appends and returns seq plus crossings", () => {
  const dir = freshDir();
  const s = open(dir);
  const r = s.post({ channel: "general", from: "a", text: "hi", id: "1", lastSeen: 0 });
  expect(r.seq).toBe(1);
...
test("dedup: repeated id returns original result and appends nothing", () => {
  const dir = freshDir();
  const s = open(dir);
  const first = s.post({ id: "dup", from: "a", text: "msg", channel: "r", lastSeen: 0 });
  const again = s.post({ id: "dup", from: "a", text: "DIFFERENT", channel: "r", lastSeen: 0 });
...
test("read and readAll implement since catch-up", () => {
  const dir = freshDir();
  const s = open(dir);
  s.post({ id: "1", from: "a", text: "one", channel: "g", lastSeen: 0 });
  s.post({ id: "2", from: "b", text: "two", channel: "dm/x/y", lastSeen: 0 });
  s.post({ id: "3", from: "c", text: "three", channel: "g", lastSeen: 0 });
...
test("crash-safe seq recovery across a reopened store", () => {
  const dir = freshDir();
  const s1 = open(dir);
  s1.post({ id: "1", from: "a", text: "x", channel: "g", lastSeen: 0 });
  s1.post({ id: "2", from: "b", text: "y", channel: "r", lastSeen: 0 });
  expect(s1.channels()).toEqual(["g", "r"]);
...
test("channel names with slashes map to subdirectories", () => {
  const dir = freshDir();
  const s = open(dir);
  s.post({ id: "1", from: "a", text: "hi", channel: "dm/dev/ana", lastSeen: 0 });
  s.post({ id: "2", from: "a", text: "hey", channel: "nested/deep/channel", lastSeen: 0 });
  expect(s.channels()).toEqual(["dm/dev/ana", "nested/deep/channel"]);
...
test("path escapes are rejected", () => {
  ...
  for (const bad of ["../evil", "a/../b", "..", "/abs"]) {
    expect(() => s.post({ id: "x", from: "a", text: "t", channel: bad, lastSeen: 0 })).toThrow();
  }
...
test("channelsFor includes channels from posts and explicit joins", () => {
  const dir = freshDir();
  const s = open(dir);
  s.post({ id: "1", from: "carol", text: "t", channel: "g", lastSeen: 0 });
  s.join("carol", "p", "other");
  const channels = s.channelsFor("carol");
...
test("subscribe notifies per append and unsubscribes", () => {
  ...
  s.post({ id: "1", from: "a", text: "t", channel: "g", lastSeen: 0 });
  expect(seen).toEqual([1]);
  un();
  s.post({ id: "2", from: "b", text: "t", channel: "g", lastSeen: 0 });
  expect(seen).toEqual([1]);
});
...
test("deliveryFor stamps mentioned from mentions", () => {
  ...
  const s = open(dir);
  s.join("dev", "p", "g");
  const r = s.post({ id: "1", from: "ana", text: "@dev check this", channel: "g", lastSeen: 0 });
...
test("mentions in a DM channel include every participant but the sender", () => {
  const dir = freshDir();
  const s = open(dir);
  const r = s.post({ id: "1", from: "ana", text: "hi", channel: "dm/ana/bob", lastSeen: 0 });
...
test("reopened store derives membership and dedup from disk", () => {
  const dir = freshDir();
  const s1 = open(dir);
  s1.post({ id: "m1", from: "eve", text: "p1", channel: "core/db", lastSeen: 0 });
  const s2 = open(dir);
  expect(s2.agents().find((a) => a.name === "eve")?.channels).toContain("core/db");
  expect(s2.readAll()).toHaveLength(1);
});

test("empty channel list on fresh store", () => {
