...
| (every send) | (none) | picks a REGISTER from who is in the channel: people against agents, counted from Slack's own marking of each member, with a `tiers` config entry overriding it. The matching block from `src/prompts/tier-<tier>.md` rides on the rewrite instruction, and the send prints which register it chose and why | n/a |
