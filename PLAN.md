...
|---|---|---|---|
| `message send` | `--target <channel>`, `--as <name>`, `--thread <id>`, `--attach <path>` (repeatable), `--mime-type <t>`, `--verify`, `--no-verify`, `--again`, `--backend <name>` | the message text on STDIN; posts it, prints each crossing as one JSON line. `--thread` replies inside that thread; each `--attach` uploads the file first and the message carries it a rewritten send reads the message back from Slack and reports what the channel stores, printing the stored text whole when it differs; `--verify` asks for that where the rewrite is off, and `--no-verify` skips it. The same draft into the same channel inside ten minutes is REFUSED, naming the ts Slack already holds, and `--again` sends it anyway | 0; 1 on a refused post or upload |
...
| `channel tier <channel> internal\|external` | (none) | writes that channel's register into the shared config, which is the operator's call to make. Prints the whole map back | 0 written; 1 on a bad tier, an unreadable config, or an unwritable one |
| (every send) | (none) | applies the REGISTER the operator set for that channel, appending `src/prompts/tier-<tier>.md` to the rewrite instruction and printing which register it used. A channel with no entry gets the careful one | n/a |
| `lint <file>...` | `--comments` | runs the send's language rules over any file, or over stdin, naming every hit with its line. `--comments` reads only the comment lines of a source file, since the rule table's own patterns contain the words it bans | 0 clean; 1 with hits |
| `rewrite [<file>]` | `--why` | runs the named file, or stdin, through the rewriter and prints what came back, with the guard verdict on stderr. Sends nothing and writes no ledger row. `--why` asks the model what is WRONG with the text, worst first, with the words quoted, and returns no rewrite | 0 rewritten; 1 unreadable, empty, refused, or no model configured |
