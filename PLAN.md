...
|---|---|---|---|
| `message send` | `--target <channel>`, `--as <name>`, `--thread <id>`, `--attach <path>` (repeatable), `--mime-type <t>`, `--verify`, `--backend <name>` | the message text on STDIN; posts it, prints each crossing as one JSON line. `--thread` replies inside that thread; each `--attach` uploads the file first and the message carries it `--verify` reads the message back from Slack and reports what the channel stores, printing the stored text whole when it differs | 0; 1 on a refused post or upload |
