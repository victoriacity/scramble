export const meta = {
  name: 'scramble-impl',
  description: 'Implement scramble (chat room for agent sessions + humans) in TS/bun at 100% coverage',
  phases: [
    { title: 'core', detail: 'store: rooms, global seq, mentions, crossings' },
...
phase('core')
const store = await agent(`${COMMON}
YOUR UNIT: src/store.ts + test/store.test.ts — the room log.

Deliver \`createStore(dir: string)\` returning the room store. Behavior, all
from DESIGN.md:
- A room is an append-only JSONL file under dir; room names may contain "/"
  (map to subdirectories; reject "..' path escapes).
- ONE global monotonically increasing seq across ALL rooms, persisted and
  crash-safe: on open, recover the next seq by scanning existing logs' maxima.
- post(input: PostInput): PostResult — appends and returns { seq, crossings }.
  Crossings are the messages in that room with seq > input.lastSeen (excluding
  the sender's own), so a sender sees what it raced with. A repeated post with
  an id already seen returns the ORIGINAL result and appends nothing (dedup).
- mentions are computed at append time by matching @name tokens against the
  known agent names; in a room whose name starts with DM_PREFIX every
  participant other than the sender counts as mentioned.
- read(room, since): Message[] and readAll(since): Message[] for catch-up.
- join(name, persona, room) records membership; membership is DERIVED (an
  agent belongs to a room once it posted or joined there) plus automatic
  membership in any dm/<name>/* room. agents(): Agent[], rooms(): string[].
- subscribe(fn) notifies listeners of each appended Message; returns an
  unsubscribe function.
- roomsFor(name) and deliveryFor(name, msg): Delivery — stamps \`mentioned\`.

Append is the only write; a message is never mutated or dropped after it is
returned. Test the crash-safe seq recovery by creating a second store over the
same dir and asserting the seq continues.`)

phase('server')
const server = await agent(`${COMMON}
YOUR UNIT: src/server.ts + test/server.test.ts — the HTTP surface over the store.

The store is already written (src/store.ts, report: ${store}). Import it.
