// src/slack-transport.ts — the REAL SlackTransport.
// Implements the seens from src/slack.ts (connect + postMessage) on bun
// built-ins only: fetch for the REST calls and an injected WebSocket for
// Socket Mode. Nothing here talks to a Slack SDK or a socket directly — the
// socket is created through the injected `createSocket` factory and fetch is
// injected too, so tests use fakes and never touch the network. The binding
// of createSocket to bun's real WebSocket lives in src/bin.ts (the edge).
import type { SlackEvent, SlackPostOptions, SlackTransport } from "./slack";
