...
CHECKS = [
    ...
    # no side directory, no index file to maintain"): a sentence that restates
    # the passage, or comments on the message itself, is padding. Delete it.
    ("redundant closer / meta-commentary", re.compile(
        r"\b(this|that|these|those) (is|are) the whole\b|"
        r"\bthis (file|skill|document|section|message|note) (is|holds|contains|covers)\b|"
        r"\b(that|this) is the point\b|\bthe (point|takeaway|upshot) is\b|"
        r"\bin (short|summary|closing)\b|\bto sum up\b|\ball told\b|"
        r"\ball along\b|\bwhich is the whole\b", re.I)),
]
